...
- **apps/web** — Application PWA React + Three.js. Charge automatiquement le projet de démonstration, gère
  l’état via Zustand, expose catalogue/propriétés/alerts, visualise le couloir par une surbrillance colorée et un
  overlay masses/barycentre interactif (toolbar + raccourcis) et orchestre les exports (BOM, DXF, OBJ, glTF, PDF).
...
- **packages/core** — Moteur métier : collisions AABB/OBB, dégagements, couloirs (visualisation + largeur
  projetée dans DXF/PDF), priorisation, masses et essieux (barycentre, charges par essieu, alertes, contributions
  par module) et exports texte (BOM, DXF, OBJ, glTF, PDF enrichi des modules influents). `evaluateProject`
  renvoie `ProjectEvaluation` consumée par le front.
