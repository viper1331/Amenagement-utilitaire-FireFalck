...
const buildContentStream = (
  ...
  if (mass.moduleContributions.length > 0) {
    pushLine('Modules influents (top 5) :');
    const topContributors = mass.moduleContributions
      .map((module) => {
        let heaviestLoad = 0;
        let heaviestIdx = -1;
        ...
        const axle = heaviestIdx >= 0 ? mass.axleLoads[heaviestIdx] : undefined;
        const share = axle && axle.load_kg > 0 ? Math.round((heaviestLoad / axle.load_kg) * 100) : 0;
        return {
          sku: module.moduleSku,
          name: module.moduleName,
          load: heaviestLoad,
          axleIndex: axle?.index ?? null,
          share,
        };
      })
      .sort((a, b) => b.share - a.share || b.load - a.load)
      .slice(0, 5);
    topContributors.forEach((contributor) => {
      const axleLabel = contributor.axleIndex === null ? 'n/a' : contributor.axleIndex.toString();
      pushLine(
        `- ${contributor.sku}: ${contributor.load.toFixed(1)} kg sur essieu ${axleLabel} (${contributor.share}% du total)`
      );
    });
  }
