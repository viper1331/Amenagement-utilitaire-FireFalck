...
import { deriveMassStatus } from '../../utils/mass';
...
export const AlertsPanel: React.FC = () => {
  ...
  const massStatus = useMemo(() => deriveMassStatus(evaluation?.massIssues), [evaluation]);
  ...
  return (
    <Panel title={t('alerts.title')} className="app-alerts">
      <div className="status-bar">
        ...
        <span>
          {massStatus.severity === null
            ? t('status.massClear')
            : t('status.massIssues', {
                severity: t(`alerts.severity.${massStatus.severity}`),
                count: massStatus.issues.length,
              })}
        </span>
        <span>
