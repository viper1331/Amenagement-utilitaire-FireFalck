...
import { deriveMassStatus, getUtilizationSeverity } from '../../utils/mass';
...
type MassOverlayRefs = {
  readonly root: THREE.Group;
  readonly barycenterGroup: THREE.Group;
  readonly barycenterLine: THREE.LineSegments;
  readonly barycenterMarker: THREE.Mesh;
  readonly axleGroup: THREE.Group;
  axleMeshes: THREE.Mesh[];
};

const MASS_BAR_BASE_HEIGHT = 120;
const MASS_BAR_HEIGHT_GAIN = 720;
const MASS_BAR_DEPTH = 240;
const MASS_BAR_WIDTH = 320;
const MASS_BAR_OPACITY = 0.55;
const MASS_COLOR_DEFAULT = 0x22c55e;
const massSeverityPalette: Record<IssueSeverity, number> = {
  info: 0x0ea5e9,
  warning: 0xf97316,
  critical: 0xef4444,
};
const BARYCENTER_COLOR_DEFAULT = 0xfacc15;
...
export const SceneCanvas: React.FC = () => {
  ...
  const massOverlayRef = useRef<MassOverlayRefs>();
  ...
  const massStatus = useMemo(
    () => deriveMassStatus(evaluation?.massIssues),
    [evaluation],
  );
  ...
  useEffect(() => {
    ...
    massGroup.name = 'mass-overlay';
    massGroup.visible = massOverlayVisible;
    massGroup.renderOrder = 2;

    const barycenterGroup = new THREE.Group();
    barycenterGroup.name = 'mass-barycenter-group';
    const barycenterMaterial = new THREE.LineBasicMaterial({
      color: BARYCENTER_COLOR_DEFAULT,
      transparent: true,
      opacity: 0.9,
      depthTest: false,
      depthWrite: false,
    });
    const barycenterGeometry = new THREE.BufferGeometry();
    const barycenterLine = new THREE.LineSegments(barycenterGeometry, barycenterMaterial);
    barycenterLine.frustumCulled = false;
    barycenterGroup.add(barycenterLine);

    const barycenterMarkerMaterial = new THREE.MeshBasicMaterial({
      color: BARYCENTER_COLOR_DEFAULT,
      transparent: true,
      opacity: 0.85,
      depthTest: false,
    ...
    const barycenterMarkerGeometry = new THREE.ConeGeometry(120, 360, 24);
    const barycenterMarker = new THREE.Mesh(barycenterMarkerGeometry, barycenterMarkerMaterial);
    barycenterMarker.rotation.x = Math.PI / 2;
    barycenterMarker.position.z = 200;
    barycenterMarker.frustumCulled = false;
    barycenterGroup.add(barycenterMarker);

    const axleGroup = new THREE.Group();
    axleGroup.name = 'mass-axle-group';

    massGroup.add(barycenterGroup);
    massGroup.add(axleGroup);
    scene.add(massGroup);
    massOverlayRef.current = {
      root: massGroup,
      barycenterGroup,
      barycenterLine,
      barycenterMarker,
    ...
    return () => {
      ...
      const massOverlay = massOverlayRef.current;
      if (massOverlay) {
        scene.remove(massOverlay.root);
        massOverlay.root.traverse((object) => {
          const meshLike = object as {
            geometry?: THREE.BufferGeometry;
            material?: THREE.Material | THREE.Material[];
          };
        ...
        massOverlayRef.current = undefined;
      }
  ...
  useEffect(() => {
    const overlay = massOverlayRef.current;
    if (!overlay) {
      return;
    }
    overlay.root.visible = massOverlayVisible;
  }, [massOverlayVisible]);
  ...
  useEffect(() => {
    ...
    const halfWidth = Math.max(250, walkwayMinWidth / 2);
    const cross = halfWidth * 0.6;
    const height = 1400;
    overlay.barycenterLine.geometry.setFromPoints([
      new THREE.Vector3(0, -halfWidth, 0),
      new THREE.Vector3(0, halfWidth, 0),
      new THREE.Vector3(-cross, 0, 0),
      new THREE.Vector3(cross, 0, 0),
      new THREE.Vector3(0, 0, 0),
      new THREE.Vector3(0, 0, height),
    ]);
    overlay.barycenterLine.geometry.computeBoundingSphere();
    overlay.barycenterMarker.scale.set(1, Math.max(0.6, walkwayMinWidth / 800), 1);
  }, [walkwayMinWidth]);
  ...
  useEffect(() => {
    ...
    const severity = massStatus.severity;
    const color = severity ? massSeverityPalette[severity] : BARYCENTER_COLOR_DEFAULT;
    const lineMaterial = overlay.barycenterLine.material as THREE.LineBasicMaterial;
    lineMaterial.color.setHex(color);
    lineMaterial.opacity = severity === 'critical' ? 0.95 : severity === 'warning' ? 0.9 : 0.85;
    lineMaterial.needsUpdate = true;
    const markerMaterial = overlay.barycenterMarker.material as THREE.MeshBasicMaterial;
    markerMaterial.color.setHex(color);
    markerMaterial.opacity = severity ? 0.9 : 0.85;
  ...
  useEffect(() => {
    ...
    const hasMass = (evaluation?.mass.totalMass_kg ?? 0) > 0;
    overlay.barycenterGroup.visible = barycenterVisible && hasMass;
    if (evaluation) {
      overlay.barycenterGroup.position.x = evaluation.mass.barycenterX_mm;
    }
  }, [barycenterVisible, evaluation]);
  ...
  useEffect(() => {
    ...
    overlay.axleMeshes.forEach((mesh) => {
      overlay.axleGroup.remove(mesh);
      mesh.geometry.dispose();
      const material = mesh.material as THREE.Material;
      material.dispose();
    });
    ...
    const width = Math.max(200, Math.min(MASS_BAR_WIDTH, walkwayMinWidth * 0.9));
    const depth = Math.max(160, Math.min(MASS_BAR_DEPTH, walkwayMinWidth * 0.6));

    evaluation.mass.axleLoads.forEach((axle) => {
      const severity = getUtilizationSeverity(axle.utilization);
      const color = severity ? massSeverityPalette[severity] : MASS_COLOR_DEFAULT;
      const geometry = new THREE.BoxGeometry(1, 1, 1);
      const material = new THREE.MeshBasicMaterial({
        color,
        transparent: true,
        opacity: MASS_BAR_OPACITY,
        depthTest: false,
        depthWrite: false,
      });
      const mesh = new THREE.Mesh(geometry, material);
      const cappedUtil = Math.max(0, Math.min(axle.utilization, 1.3));
      const height = MASS_BAR_BASE_HEIGHT + MASS_BAR_HEIGHT_GAIN * cappedUtil;
      mesh.scale.set(width, depth, height);
      mesh.position.set(axle.position_mm, 0, height / 2 + 5);
      mesh.frustumCulled = false;
      overlay.axleGroup.add(mesh);
      overlay.axleMeshes.push(mesh);
    });
  }, [evaluation, walkwayMinWidth]);
