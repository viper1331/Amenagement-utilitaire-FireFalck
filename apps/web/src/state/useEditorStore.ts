...
export interface EditorState {
  ...
  readonly massOverlayVisible: boolean;
  readonly barycenterVisible: boolean;
  ...
  setMassOverlayVisible: (visible: boolean) => boolean;
  toggleMassOverlayVisible: () => boolean;
  setBarycenterVisible: (visible: boolean) => boolean;
  toggleBarycenterVisible: () => boolean;
...
const applyProjectUpdate = (
  ...
  const walkwayMinWidth = clampWalkwayWidth(next.settings.walkway.minWidth_mm, vehicle);
  next.settings.walkway.minWidth_mm = walkwayMinWidth;
  const walkwayVisible = next.settings.walkway.showOverlay ?? true;
  next.settings.walkway.showOverlay = walkwayVisible;
  const massSettings = next.settings.mass ?? { showOverlay: true, showBarycenter: true };
  next.settings.mass = massSettings;
  const massOverlayVisible = massSettings.showOverlay ?? true;
  const barycenterVisible = massSettings.showBarycenter ?? true;
...
export const useEditorStore = create<EditorState>((set, get) => ({
  ...
  walkwayMinWidth: 500,
  walkwayVisible: true,
  massOverlayVisible: true,
  barycenterVisible: true,
  toasts: [],
  history: { past: [], future: [] },
  setProject: (project) => {
    ...
    const massSettings = parsed.settings.mass ?? { showOverlay: true, showBarycenter: true };
    parsed.settings.mass = massSettings;
    const massOverlayVisible = massSettings.showOverlay ?? true;
    const barycenterVisible = massSettings.showBarycenter ?? true;
    set({
      ready: true,
  ...
  setWalkwayVisible: (visible) => {
    applyProjectUpdate(
      ...
      get,
      (project) => {
        project.settings.mass = project.settings.mass ?? { showOverlay: true, showBarycenter: true };
  ...
  setMassOverlayVisible: (visible) => {
    applyProjectUpdate(
      set,
      get,
      (project) => {
        project.settings.mass = project.settings.mass ?? { showOverlay: true, showBarycenter: true };
        project.settings.mass.showOverlay = visible;
      },
      false,
    );
    set({ massOverlayVisible: visible });
    return visible;
  },
  toggleMassOverlayVisible: () => {
    const next = !get().massOverlayVisible;
    get().setMassOverlayVisible(next);
    return next;
  },
  setBarycenterVisible: (visible) => {
    applyProjectUpdate(
      set,
      get,
      (project) => {
        project.settings.mass = project.settings.mass ?? { showOverlay: true, showBarycenter: true };
        project.settings.mass.showBarycenter = visible;
      },
      false,
    );
    set({ barycenterVisible: visible });
    return visible;
  },
  toggleBarycenterVisible: () => {
    const next = !get().barycenterVisible;
    get().setBarycenterVisible(next);
    return next;
  },
